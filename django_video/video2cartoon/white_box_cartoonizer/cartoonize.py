...
BASE_DIR = os.path.join(BASE_DIR / 'media/uploaded_video')
...
class WB_Cartoonize:
    ...
    def process_video(self, fname, frame_rate):
        ...
        output_fname = os.path.abspath('{}/{}-{}.mp4'.format((BASE_DIR), str(uuid.uuid4())[:7], os.path.basename(fname).split('.')[0]))
        
        print(fname)
        print(output_fname)

        out = skvideo.io.FFmpegWriter(output_fname, inputdict={
                                      '-r': frame_rate}, outputdict={'-r': frame_rate})
        ...
        final_name = '{}final_{}'.format(fname.replace(
            os.path.basename(fname), ''), os.path.basename(output_fname))

        p = subprocess.Popen(
            ['ffmpeg', '-i', '{}'.format(output_fname), "-pix_fmt", "yuv420p", final_name])
        p.communicate()
        p.wait()
...
#     wbc = WB_Cartoonize(os.path.abspath(
#         'white_box_cartoonizer/saved_models'), gpu)
